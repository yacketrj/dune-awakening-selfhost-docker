          ...
          {packageItemsOpen && <div className="playerAdmin_toggleBody"><div className="package-items-picker-panel">
            ...
            <div className="action-line">
              ...
              {(() => {
                ...
                return filteredAugs.length === 0 ? (augmentLimit(packageDraft.itemName) === 0 ? <label><span className="playerAdmin_note">Augments only available for weapons and armor</span></label> : null) : augmentLimit(packageDraft.itemName) === 0 ? <label><span className="playerAdmin_note">Augments only available for weapons and armor</span></label> : <label>Augments<select className="augment-picker" multiple value={packageDraft.augments} onChange={(event) => { const selected = Array.from(event.target.selectedOptions, (opt) => opt.value).slice(0, augmentLimit(packageDraft.itemName)); setPackageDraft({ ...packageDraft, augments: selected }); }} style={{ minWidth: 320, maxWidth: 420, height: 60 }}>{filteredAugs.map((aug) => <option key={aug.id} value={aug.id}>{aug.name}</option>)}</select></label>;
              })()}
